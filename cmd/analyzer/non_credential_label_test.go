...
func TestACredentialWordCannotIntroduceAName(t *testing.T) {
	...
	kept := []string{
		...
		`auth: "sha256" = "` + secret + `"`,
		`password."md5" = "` + secret + `"`,
		// a list or a bracket does not hide the word in front of it
		`signing, "sha256" = "` + secret + `"`,
		`password = {"sha256": "` + secret + `"}`,
		`signing {"sha256": "` + secret + `"}`,
		`password = ["sha256": "` + secret + `"]`,
		...
		// the chain continues past a container that is not a credential word
		`{"webhook_secret": {"primary": {"sha256": "` + secret + `"}}}`,
		`{"password": {"a": {"b": {"sha256": "` + secret + `"}}}}`,
		// an unquoted sibling value is stepped over like a quoted one
		...
		"password: {primary: {\"sha256\": \"" + secret + "\"}}",
		"webhook_secret: {\n  primary: {\n    sha256: " + secret + "\n  }\n}",
		// a container key that opens its own line, indented or not
		"webhook_secret:\nprimary: {sha256: " + secret + "}",
		"webhook_secret:\nprimary: {\"sha256\": " + secret + "}",
		"webhook_secret: {\nprimary: {sha256: " + secret + "}\n}",
		// indentation is not what puts a container key there
		"webhook_secret:\n  primary: {sha256: " + secret + "}",
		"webhook_secret:\n  primary: {\"sha256\": " + secret + "}",
		"password:\n\tprimary: {sha256: " + secret + "}",
